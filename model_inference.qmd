...
- [ ] Write introductionand explain that this should be done in colab
...
Before starting, make sure you are connected to a GPU runtime (Runtime -> Change Runtime Type).

The first thing we need to do is install the relevant packages, some packages need to be installed each time we start a new session an some can just be loaded in. All we need to install in this notebook are datasets and tqdm. Datasets is an alternative to pandas for efficient computation, which we will discuss more later, and tqdm is going to be used later so that we can get some progress bars during computation.

Python commpands prefaced by "%%" are referred to as "cell magic" commands, `%%capture` is an example of one such command which stops a cell from printing all output information when run. Installing packages can output *a lot* of information, which I tend to supress for the sake of a tidy script.

To be fair, sometimes it is useful information, particularly down the line when code suddenly stops working and you don't know why - the culprit is usually differen changing dependencies in the installed packages, information that is output in the printed content.

```{{python}}
%%capture
!pip install datasets # useful to use instead of pandas for efficient model execution
!pip install tqdm # for tracking computation progress later in the notebook
!pip install pyjanitor
```

We also want to mount our google drive so that we can load from and save to our drive.
...
The model needs two inputs, both of which are included in the `tokenised_dataset`:
* input_ids
* attention_mask
...
* softmax
$$\text{softmax}(x_i) = \frac{e^{x_i}}{\sum_je^{x_j}}$$
* sigmoid
